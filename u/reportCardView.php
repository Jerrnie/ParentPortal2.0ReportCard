...
<head>
  ...
  <title>Registration | Parent Portal</title>
                ...
                if (mysqli_num_rows($result1) > 0) {
                  while ($row = mysqli_fetch_array($result1)) {
                    ...
                    if ($haveMatch) {
                      ...
                      echo '       <a class="btn btn-primary iconsize" title="View Report Card"  href="rcViewer.php?page=' . $row[1] . '&name=' . $name . '">';
                      echo '           <i class="d-flex justify-content-center fas fa-eye">';
                      echo '           </i>';
                      echo '           <span id="view' . $ctr . '"></span>';
                      echo '       </a>';
                      echo '       <a href="#" title="Remove Report Card" class="btn delete iconsize btn-danger" title="Delete" id="delete' . $ctr . '" rowIdentifier="row' . $ctr . '"  value="' . $row[1] . '" >';
                      ...
                      echo '<td class="text-center">';
                      echo '       <button disabled class="btn btn-primary btn-sm " title="No Report Card Available" id="veButton' . $ctr . '"  href="?">';
                      echo '           <span id="view' . $ctr . '"></span>Report Card not available';
                      echo '       </button>';
                    }
        ...
        <div class="card-body">
          <div class="tab-content" id="custom-tabs-two-tabContent">
            ...
            <div class="tab-pane fade" id="custom-tabs-two-profile" role="tabpanel" aria-labelledby="custom-tabs-two-profile-tab">
              <b>Reminder:</b>
              <ol>
                <li>All report cards must be placed in a single folder.</li>
                <li>Individual report card must be named the same as the student code.</li>
                <li>All report cards must be in a PDF file format.</li>
        ...
        $.ajax({
          ...
          success: function() {
            Swal.fire({
              ...
              document.location.href = 'reportCardView.php';
            });
          },
  ...
  if (in_array($fileActualExt, $allowed)) {
    ...
    if ($fileError == 0) {
      if ($fileSize < 5000000) {
        ...
        $fileDestination = 'RC/' . $fileNameNew1;

        if (file_exists('RC/' . $fileNameNew1)) {
          unlink('RC/' . $fileNameNew1);
        }


        $checkRecord1 = mysqli_query($conn, "SELECT firstName, middleName, lastName FROM tbl_student WHERE studentCode  = '" .  $_POST['studentcode']  . "' ");
        ...
        $insertauditQuery = "Insert into tbl_audittrail (userID, fname, lname, activity, activitydate,schoolYearID) Values  ('" .  $userID . "', '" .  $userFname . "', '" .  $userLname . "', 'Uploads a report card for student named ' '" . $fileName . "', '$date','" . $schoolYearID . "')";
        mysqli_query($conn, $insertauditQuery);

        header('Location: reportCardView.php?uploadingSuccess');
      } else {

        header('Location: reportCardView.php?bigFile');
      }
    } else {
      header('Location: reportCardView.php?error');
    }
  } else {
    header('Location: reportCardView.php?notFound');
  }
}
...
if (isset($_POST['upload'])) {
  ...
  foreach ($_FILES['files']['name'] as $ii => $name) {
    ...
    if (in_array($fileActualExt, $allowed)) {
      ...
      header('Location: reportCardView.php?error2');
    } else {
      header('Location: reportCardView.php?pdfRequired');
    }
  }
}
...
if (isset($_REQUEST['pdfRequired'])) {
  displayMessage("Warning", "Warning", "Files must be on PDF format!");
}
