                ...
                if (mysqli_num_rows($result1) > 0) {
                  while ($row = mysqli_fetch_array($result1)) {
                    ...
                    if ($haveMatch1) {
                      ...
                      echo '       <a class="btn btn-primary iconsize" title="View Statement of Account" href="soaViewer.php?page=' . $row[1] . '&name=' . $name . '">';
                      echo '           <i class="d-flex justify-content-center fas fa-eye">';
                      echo '           </i>';
                      echo '           <span id="view' . $ctr . '"></span>';
                      echo '       </a>';
                      echo '       <a href="#" title="Remove Statement of Account" class="btn delete iconsize btn-danger" title="Delete" id="delete' . $ctr . '" rowIdentifier="row' . $ctr . '"  value="' . $row[1] . '" >';
                      ...
                      echo '<td class="text-center">';
                      echo '       <button disabled class="btn btn-primary btn-sm " title="No SOA Available" id="veButton' . $ctr . '"  href="?">';
                      echo '           <span id="view' . $ctr . '"></span>SOA not available';
                      echo '       </button>';
                    }
        ...
        <div class="card-body">
          <div class="tab-content" id="custom-tabs-two-tabContent">
            ...
            <div class="tab-pane fade" id="custom-tabs-two-profile" role="tabpanel" aria-labelledby="custom-tabs-two-profile-tab">
              <b>Reminder:</b>
              <ol>
                <li>All statement of accounts must be placed in a single folder.</li>
                <li>Individual statement of account must be named the same as the student code.</li>
                <li>All statement of accounts must be in a PDF file format.</li>
        ...
        $.ajax({
          ...
          success: function() {
            Swal.fire({
              ...
              document.location.href = 'soaView.php';
            });
          },
  ...
  if (in_array($fileActualExt, $allowed)) {
    ...
    if ($fileError == 0) {
      if ($fileSize < 5000000) {
        ...
        $fileDestination = 'SOA/' . $fileNameNew1;

        if (file_exists('SOA/' . $fileNameNew1)) {
          unlink('SOA/' . $fileNameNew1);
        }
        ...
        $insertauditQuery = "Insert into tbl_audittrail (userID, fname, lname, activity, activitydate,schoolYearID) Values  ('" .  $userID . "', '" .  $userFname . "', '" .  $userLname . "', 'Uploads a statement of account for student named ' '" . $fname . " ' '" . $mname . " ' '" . $lname . "', '$date','" . $schoolYearID . "')";
        mysqli_query($conn, $insertauditQuery);

        move_uploaded_file($fileTmpName, $fileDestination);
        header('Location: soaView.php?uploadingSuccess');
      } else {

        header('Location: soaView.php?bigFile');
      }
    } else {
      header('Location: soaView.php?error');
    }
  } else {
    header('Location: soaView.php?notFound');
  }
}
...
if (isset($_POST['upload'])) {
  ...
  foreach ($_FILES['files']['name'] as $ii => $name) {
    ...
    if (in_array($fileActualExt, $allowed)) {
      ...
      header('Location: soaView.php?error2');
    } else {
      header('Location: soaView.php?pdfRequired');
    }
  }
}
