                  ...
                  <form name="myform" action="" method="post">
                    <div class="col-mb-3">
                      <div class="form-group">
                        <div class="row">
                          <div class="col-sm-5">
                            <textarea readonly list="user" name="compose-to" id="composeTo" type="text" class="form-control" placeholder="Student List:" required><?php echo isset($_POST['fname']) ? $_POST['fname'] : '' ?></textarea>
                          </div>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-md" role="document">
      <div class="modal-content">
        ...
        <ul class="nav nav-tabs" id="custom-tabs-two-tab" role="tablist">
          <li class="nav-item">
            <a class="nav-link active tab1" id="custom-tabs-two-home-tab" data-toggle="pill" href="#parentlist" role="tab" aria-controls="custom-tabs-two-home" aria-selected="true">List of Parents</a>
          </li>
        <li class="nav-item">
            <a class="nav-link tab2" id="custom-tabs-two-profile-tab" data-toggle="pill" href="#sectionlist" role="tab" aria-controls="custom-tabs-two-profile" aria-selected="false">List of Section</a>
        </li>
        </ul>
          ...
          <div class="modal-body" id="body">
            <div class="tab-content">
              ...
              <table id="example2" class="table table-borderless table-striped table-layout: fixed">
                  ...
                  <tbody>
                    <?php
                    $query = "SELECT a.sectionName, b.sectionID, c.fullName, c.userID, d.sectionYearLevel
                              FROM tbl_sections a
                              LEFT JOIN tbl_student b ON a.sectionID = b.sectionID                              
                              LEFT JOIN tbl_parentuser c ON b.userID = c.userID 
                              LEFT JOIN tbl_sections d ON d.sectionID = b.sectionID
                              WHERE c.schoolYearID = '" . $schoolYearID . "' ORDER BY d.sectionYearLevel asc ";
                    $result = mysqli_query($conn, $query);
                    ...
                    if ($result) {
                      ...
                      if (mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_array($result)) {
                          ...
                          if (in_array($row[1],  $addsecid)) {
                          } else {
                            $addsecid[] = $row[1];

                            if ($secid == $row[1]) {
                              if (in_array($row[0],  $addsecname)) {
                              } else {
                    ?>
                                <tr>
                                  <td><input id="member_select" class="get_value" rName="<?php echo $secname ?>" type="checkbox" name="pid[]" value="<?php echo $puserid . ' ' ?>"></td>
              ...
              <div class="modal-footer justify-content-between">
                ...
                <!-- <button id="Up" type="submit" class="btn btn-primary" name="Up">Oku</button> -->
                <button id="Ok" type="submit" class="btn btn-primary" name="Ok" value="true">Ok</button>
              </div>
              </form>
            </div>
...
if (isset($_POST['send'])) {
  ...
  //$_POST['compose-to'] = mysqli_real_escape_string($conn, stripcslashes($_POST['compose-to']));
