...
if (in_array($ext, $allowed)) {
	if ($fileSize > 10 * MB) {
		...
		while ($excel->getActiveSheet()->getCell('A' . $i)->getValue() != "") {
			...
			$ParentName2		= mysqli_real_escape_string($conn, stripcslashes(trim($excel->getActiveSheet()->getCell('N' . $i)->getValue())));
			$MobileNo2		= mysqli_real_escape_string($conn, stripcslashes(trim($excel->getActiveSheet()->getCell('O' . $i)->getValue())));
			$Email2			= mysqli_real_escape_string($conn, stripcslashes(trim($excel->getActiveSheet()->getCell('P' . $i)->getValue())));
...
WHERE c.userID = '" . $userID . "'";
			...
			where studentCode = '" . $studentCode . "'";
					...
					//change the handler of student
						...
						$insertQuery1 = "Insert into tbl_student
						(
						userID,
						studentID,
						studentCode,
					...
					$noNewStudent++;
					$sql = "sELECT a.studentID FROM tbl_student AS a WHERE studentCode = '".$studentCode."'";


					$result = mysqli_query($conn, $sql);
					$pass_row = mysqli_fetch_assoc($result);
					$userID = $pass_row['studentID'];

						$hashedPassword = password_hash($MobileNo, PASSWORD_DEFAULT);
						$insertQuery = "Insert into tbl_parentuser
						(mobile,email,password,isEnabled,userType,sqID,isReset,fullName,schoolYearID,studentID)
						VALUES
						('" . $MobileNo . "','" . $Email . "','" . $hashedPassword . "','1','P','1','1','" . $ParentName . "','".$schoolYearID."','".$studentID."'
					)";
						mysqli_query($conn, $insertQuery);
						$noNewAccounts++;
					...
					$sql = "sELECT b.mobile, a.userID FROM tbl_student AS a inner join tbl_parentuser as b on a.userID=b.userID WHERE b.mobile = '" . $MobileNo . "'";
					$result = mysqli_query($conn, $sql);
					$totalrows = mysqli_num_rows($result);
					...
					$studID = $pass_row['studentID'];
						...
						(mobile,email,password,isEnabled,userType,sqID,isReset,fullName,schoolYearID,studentID)
						VALUES
						('" . $MobileNo . "','" . $Email . "','" . $hashedPassword . "','1','P','1','1','" . $ParentName . "','".$schoolYearID."','".$studID."'
					)";
						mysqli_query($conn, $insertQuery);

						$sql = "sELECT a.userID FROM tbl_parentuser AS a WHERE mobile = '".$MobileNo."' AND schoolYearID = '".$schoolYearID."'";

						$result = mysqli_query($conn, $sql);
						$pass_row = mysqli_fetch_assoc($result);
						$userID = $pass_row['userID'];

						$insertQuery4 = "Insert into tbl_StudentTransaction
						(userID,studentID,schoolYearID)
						VALUES
						('" . $userID . "','" . $studID . "','".$schoolYearID."')";
						mysqli_query($conn, $insertQuery4);
	
						$noNewAccounts++;

						$hashedPassword2 = password_hash($MobileNo2, PASSWORD_DEFAULT);
						$insertQuery3 = "Insert into tbl_parentuser
						(mobile,email,password,isEnabled,userType,sqID,isReset,fullName,schoolYearID,studentID)
						VALUES
						('" . $MobileNo2 . "','" . $Email2 . "','" . $hashedPassword2 . "','1','P','1','1','" . $ParentName2 . "','".$schoolYearID."','".$studID."'
					)";
						mysqli_query($conn, $insertQuery3);

						$sql = "sELECT a.userID FROM tbl_parentuser AS a WHERE mobile = '".$MobileNo2."' AND schoolYearID = '".$schoolYearID."'";

						$result = mysqli_query($conn, $sql);
						$pass_row = mysqli_fetch_assoc($result);
						$userID2 = $pass_row['userID'];

						$insertQuery5 = "Insert into tbl_StudentTransaction
						(userID,studentID,schoolYearID)
						VALUES
						('" . $userID2 . "','" . $studID . "','".$schoolYearID."')";
						mysqli_query($conn, $insertQuery5);
