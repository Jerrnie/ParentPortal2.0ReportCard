...
if (in_array($ext, $allowed)) {
    ...
    array_push($faileds, array(
		...
		#Get all ID
		$checkRecord = mysqli_query($conn, "sELECT a.Personnel_code, b.Mobile FROM tbl_Personnel as a inner join tbl_parentuser as b on b.pID = a.Personnel_Id");
		...
		while ($excel->getActiveSheet()->getCell('A' . $i)->getValue() != "") {
			...
			if (!$hasError) {
				...
				if (in_array($Personnel_code, $list_personnelCode)) {
					...
					$sql = "sELECT a.Personnel_Id,b.SchoolYearID FROM tbl_Personnel AS a,tbl_PersonnelTransaction AS b WHERE a.Personnel_Id=b.Personnel_Id AND a.Personnel_code = '" . $Personnel_code . "' AND b.SchoolYearID = '".$schoolYearID."'";
					$result = mysqli_query($conn, $sql);
					$pass_row = mysqli_fetch_assoc($result);
					$oldID = $pass_row['Personnel_Id'];
					$oldSY = $pass_row['SchoolYearID'];
					
					$sql2 = "SELECT b.mobile,b.pID FROM tbl_Personnel AS a, tbl_parentuser AS b, tbl_PersonnelTransaction AS c WHERE a.Personnel_Id=b.pID AND c.Personnel_Id=a.Personnel_Id AND b.pID = '".$oldID."'  AND b.usertype='E' AND c.SchoolYearID = '".$schoolYearID."'";
					...
					if ($totalrows > 0) {
						...
						if ($schoolYearID == $oldSY)
						{
							...
							$pID = $pass_row['pID'];
								...
								$updateQuery = "update tbl_Personnel
								set
							...
							$insertQuery2 = "Insert into tbl_PersonnelTransaction
							(
								Personnel_Id,
								schoolYearID
							)
