...
<head>
  ...
  <title>Appointment History | Parent Portal </title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <!-- Default box -->
          ...
          $sql = "select A.RequestRemarks, A.DateTimeRequested, A.status,
                  Concat(U.lname,', ',U.fname,' ',U.mname) as ParentName,
                  DAte(S.DateSchedule) as DateSchedule, S.occupied, S.WebLink, S.SchedTimeFrom, S.SchedTimeTo,
                  P.Personnel_code,
                  concat(P.Lname,', ',P.Fname,' ', P.Mname) as PersonnelName,P.Position,A.Appoint_Id,A.Reason,A.ReadTag,A.Appoint_Id
                  ...
                  where (A.status ='Approved' or A.status ='Denied' or A.status ='Pending')
                   and U.userID = $user_check
            ...
            $ctr=0;
              if (mysqli_num_rows($result1) > 0) {
                while($row = mysqli_fetch_array($result1)){
                  ...
                  //$DateSchedule   = $row[4];
                  $DateSchedule = date_format(date_create($row[4]), 'm/d/Y');
                 // $SchedTimeFrom   = $row[7];
                  $SchedTimeFrom = date_format(date_create($row[7]), 'h:i A');
                  $SchedTimeTo = date_format(date_create($row[8]), 'h:i A');
                 // $SchedTimeTo    = date("h : i : s A");
                  $ParentName    = $row[3];
                  $RequestRemarks  = $row[0];
                  $PersonnelName   = $row[10];
                  //$DateTimeRequested  = $row[1];  
                  $DateTimeRequested = date_format(date_create($row[1]), 'm/d/Y h:i A');
                  $status= $row[2];
                  $Reason   = $row[13];
                  $ReadTag   = $row[14];
                  $AppointID   = $row[15];
          ...
          echo"<tr class='tRow' id='row".$ctr."' style='background-color:#ebebeb;'>";
                  ...
                  echo"<td><h6>";
                  echo'<a href="ReadAppointment.php?page='.$row[15].'">';
                  echo "$DateSchedule";
                  echo '</a>';
                  echo"</td><h6>";
                  echo"<td><h6>";
