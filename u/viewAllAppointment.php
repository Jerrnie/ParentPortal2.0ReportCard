          ...
          <div class="card">
            <div class="card-header">
              ...
              <a href="?" type="button" class="btn btn-success add-button buttonDelete ">
                ...
                <span class=" fa fa-plus-square">&nbsp&nbsp</span>Create New Appointments
                </a>
              </p>
            </div>
            <!-- /.card-header -->
            <div class="card-body" style="width: 100%;">
              <table id="example1" class="table table-bordered" style="table-layout: fixed; width: 100%;">
                ...
                <tbody>
          <?php
          $sql = "SELECT  a.* ,b.SchedTimeFrom,b.SchedTimeTo,b.WebLink 
          FROM tbl_appointment AS a  INNER JOIN tbl_PersonnelSched AS b 
          ON b.PersonnelSched_Id = a.PersonnelSched_Id";
            ...
            $ctr=0;
              if (mysqli_num_rows($result1) > 0) {
                while($row = mysqli_fetch_array($result1)){
                  ...
                  $Appoint_Id   = $row[0];
                  $PersonnelSched_Id    = $row[1];
                  $ParentId     = $row[2];
                  $RequestRemarks         = $row[3];
                  $DateTimeRequested = date_format(date_create($row[4]),"M d, Y");
                  $SchedTimeFrom    = $row[9];
                  $SchedTimeTo      = $row[10];
                  $WebLink  = $row[11];
                  $status= $row[5];
